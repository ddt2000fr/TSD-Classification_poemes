...
def load_corpus(corpus_dir, max_per_author=111):
    texts = []
    labels = []
    authors = ['baudelaire', 'verlaine', 'rimbaud']
...
print(classification_report(y_test, y_pred_nb,
      target_names=['baudelaire', 'verlaine', 'rimbaud']))

cm_nb = confusion_matrix(y_test, y_pred_nb,
        labels=['baudelaire', 'verlaine', 'rimbaud'])
plt.figure(figsize=(8, 6))
sns.heatmap(cm_nb, annot=True, fmt='d',
            xticklabels=['Baudelaire', 'Verlaine', 'Rimbaud'],
            yticklabels=['Baudelaire', 'Verlaine', 'Rimbaud'])
plt.title('Matrice de confusion - Naive Bayes')
plt.ylabel('Réel')
plt.xlabel('Prédit')
plt.savefig('confusion_matrix_nb.png')
print("Matrice de confusion sauvegardée: confusion_matrix_nb.png")
